...
Please be aware that there are elements of this program that were created by other developers and researchers, which may specify different license terms, including *non-commercial use*. Please respect their work by respecting their licences.

Specifically these components are:

**German word concreteness database**

Used to calculate average concreteness ratings for nouns (stored in ```program_files``` folder as ```concrete_de.csv```).
...
### **Achtung!**
It is critical that these folders are included in the program directory and named exactly as above. 
...
A CSV file (comma delimited) will be created in the ```usr_output``` folder with the name ```data_array_date-and-time.csv```. This can be loaded into the statistical package of your choice (R, SPSS, etc.). Ensure add any additional independent variables (eg. difficulty level, text genre, etc.). 
...
### WORD_COUNT
The number of words in the text. Contractions are counted as one.

### SENT\_LENm, SENT_LENsd
...
### NOUN\_OL1m, NOUN\_OL1sd, NOUN\_OLm, NOUN_OLsd

### VERB\_OL1m, VERB\_OL1sd, VERB\_OLm, VERB_OLsd

### CONTENT\_OL1m, CONTENT\_OL1sd, CONTENT\_OLm, CONTENT_OLsd
...
### PRONOUN
The percentage of total words in the text that are tagged as a personal pronoun. Pronouns are indicative of a narrative genre which, overall tends to be easier to read than informative or argumentative genres.
